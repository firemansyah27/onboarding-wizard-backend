...
async function fastifyAxios(fastify, opts) {
            ...
            if (clients[name].interceptors) {
                if (clients[name].interceptors.request && clients[name].interceptors.errorRequest) {
                    axiosInstance.interceptors.request.use(
                        clients[name].interceptors.request,
                        clients[name].interceptors.errorRequest,
                    );
                }

                if (
                    clients[name].interceptors.response &&
                    clients[name].interceptors.errorResponse
                ) {
    ...
    fastify.decorate("axios", async function (request, reply, done) {
        try {
...
const fatifyaxios = fp(fastifyAxios, {
    fastify: ">= 2.0.0",
    name: "fastify-axios",
});

export { fatifyaxios as default };
